---
slug: contact-nui-ba-1971-10-21
title: Contact at Nui Ba Mountains — Death of PFC Makowski
status: published
type: contact
date: "1971-10-21"
date_end: 1971-10-22T00:00:00.000Z
date_known: true
location: Nui Ba Mountains, Binh Tuy Province, RVN
units:
  primary:
    - slug: d-co-2-8-cav
      name: D Company, 2nd Battalion, 8th Cavalry (Airmobile), "The Angry Skippers"
      role: Engaged; follow-through sweep of bunker complex day two
platoons:
  - name: Range Platoon
    call_sign: "Rifle Range"
    number: 2
    role: Lead element on contact; follow-through sweep October 22
  - name: Cat Platoon
    call_sign: "Wild Cat"
    number: 3
    role: 400 meters distant at time of contact; CO Kutter was with Cat Platoon
platoon_call_signs:
  - number: 1
    name: White Cat
  - number: 2
    name: Rifle Range
  - number: 3
    name: Wild Cat
  - number: 4
    name: Lethal Weapons
casualties:
  kia:
    - slug: makowski-william
      name: PFC William Makowski
      date: 1971-10-21T00:00:00.000Z
      note: >
        M-60 Machine Gunner, Range Platoon. KIA in contact; stayed up to return fire with M-60
        after enemy ambush; hit in the neck by shrapnel severing the jugular vein. Died at the
        perimeter after being carried back by Hilts and Blais. FSB subsequently named Makowski
        in his honor.
  wia:
    - name: Escue (first name unknown)
      role: Dog handler
      note: WIA; non-infantry specialist attached to Range Platoon; first name not yet confirmed
    - slug: blais-dizzy
      name: Jean "Dizzy" Blais
      role: RTO, Range Platoon
      note: WIA; shrapnel splinter in facial tissue from claymore detonation
    - slug: hilts-doug
      name: Doug Hilts
      role: Squad Leader / Point Man, Range Platoon
      note: WIA; shrapnel ricocheted through helmet causing concussion; helmet liner saved him
tagged:
  - slug: kutter-wolf
    name: Capt. Wolf Kutter
    note: CO, D Co., "Skipper 6"; with Cat Platoon ~400m away at time of contact; flew back to Firebase Jefferies with body
  - slug: blagg-thomas
    name: Col. Tom Blagg
    note: Battalion commander; flew his own helicopter to extract Makowski's body when medevac refused to fly
accounts:
  - slug: kutter-wolf-account-102171
    type: account
    status: draft
    access: restricted
    note: Unpublished memoir; not to be published without explicit authorization from Kutter or estate
  - slug: makowski-memorial-booklet-2021
    type: memorial
    status: published
    access: restricted
    note: >
      Memorial booklet produced for 50th anniversary graveside service, Oct 21, 2021.
      Contains tactical narrative with map, damaged photo of Blais and Makowski before the
      mission, and Kutter's commemoration speech. Draws on firsthand accounts of Hilts and
      Blais. Restricted to family/unit use; not for public reproduction without consent.
related_events:
  - slug: contact-nui-ba-1971-10-22
    relationship: continuation
    note: Follow-through sweep of bunker complex on October 22; record not yet created
open_questions:
  - id: oq-01
    publish: true
    question: |
      PFC William Makowski was killed in contact on October 21, 1971 in the Nui Ba Mountains. If you were with Range Platoon at that time, or knew Makowski, we want to hear from you.
  - id: oq-02
    publish: true
    question: |
      A dog handler named Escue was wounded in this engagement. We do not have his first name or further information. If you can identify him, please contact us.
  - id: oq-03
    publish: false
    resolved: true
    resolution: >
      FSB Makowski confirmed — named by the unit in his honor. Kutter speech (2021) explicitly
      states "We named a Firebase for Bill, Firebase Makowski, for that reason in his honor."
      Timing: after the follow-through sweep on October 22, before the company returned to
      Firebase Jefferies.
    question: |
      Confirm FSB Makowski naming — was the fire support base formally named after PFC Makowski, and when? Check battalion records.
  - id: oq-04
    publish: false
    resolved: true
    resolution: >
      Memorial booklet (2021) provides independent corroboration from Hilts and Blais —
      both present on October 21, 1971. Event record may now be considered fully sourced
      for narrative purposes. Kutter memoir remains restricted separately.
    question: |
      Kutter memoir is the only account currently available for this event. Locate and contact Hilts and Blais for independent accounts before publishing this event record.
archivist_notes:
  created: 2026-05-11T00:00:00.000Z
  last_updated: 2026-05-27
  session: 18
  sources:
    - kutter-wolf-account-102171
    - makowski-memorial-booklet-2021
    - McGrew calendar Oct 21-22 and Oct 23
  access_note: >
    Two sources now available. The Kutter memoir remains permanently restricted.
    The memorial booklet is a family/unit document — narrative details from it may
    be published but the document itself should not be reproduced without consent.
    McGrew calendar provides independent date corroboration (Oct 21, Oct 22, and
    "Back to Jeffries" Oct 23).
contains:
  - hilts-doug
  - blais-dizzy
---

## Summary

On October 21, 1971, D Company, 2nd Battalion, 8th Cavalry made contact with enemy forces in the Nui Ba Mountains, Binh Tuy Province. PFC William Makowski was killed in action. His squad leader Doug Hilts and RTO Jean "Dizzy" Blais were wounded. A dog handler named Escue was also wounded. The following day the unit conducted a follow-through sweep of an enemy bunker complex in the same area, then returned to Firebase Jefferies where a ceremony was held for PFC Makowski. Howard McGrew's calendar records contact on both October 21 and October 22, and "Back to Jeffries" on October 23, providing independent date corroboration.

## Sequence of Events

Bill's squad — under Squad Leader Doug Hilts, also serving as Point Man, with RTO Jean "Dizzy" Blais — conducted the final security patrol around Range Platoon's Night Defense Perimeter at approximately 6 p.m. Fresh cuttings had been found in the jungle that day, a sign that an enemy force was operating nearby. The squad moved cautiously.

The enemy opened fire with AK-47s and an RPG; a claymore mine was also detonated. After the ambush opened, Bill stayed up and returned fire with his M-60 Machine Gun. He was hit in the neck by shrapnel, severing his jugular vein. Hilts (shrapnel through his helmet, causing a concussion) and Blais (shrapnel in his face) dragged Bill behind two fallen trees and carried him approximately 70 yards back to the perimeter, where the platoon medic treated him. The bleeding could not be stopped. Bill died among his platoon mates.

The medevac helicopter at Firebase Jefferies refused to fly due to the ongoing firefight. Battalion commander Col. Tom Blagg flew his own helicopter to extract Bill's body. A ceremony was held at Firebase Jefferies the following day — Bill's helmet atop an M-16 rifle with fixed bayonet, his muddy jungle boots in front, final Roll Call, and Taps — before the unit conducted the follow-through sweep of the bunker complex on October 22.

## Context and Notes

**Cause of death:** Shrapnel (claymore/RPG detonation), severing the jugular vein. DoD DCAS lists cause as "small arms fire" — a general hostile-action category that encompasses all weapon types in the engagement.

**FSB naming:** A fire support base was named FSB Makowski in his honor. Confirmed by Kutter's 2021 commemoration speech: "We named a Firebase for Bill, Firebase Makowski, for that reason in his honor."

**Firebase Jefferies:** The battalion base at the time of this operation was Firebase Jefferies, likely named after WO1 Gabriel Jefferies (KIA April 24, 1971 in the FSB Fontaine crash).

**Dog handler casualty:** Escue's first name has not been confirmed. His presence indicates a scout dog team was integrated into Range Platoon's operation.
